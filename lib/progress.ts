...
export function getProgressSummary(sessions: WorkoutSession[]): ProgressSummary {
  ...
  const totalVolume = completedSessions.reduce(
    (sum, session) => sum + session.setLogs.reduce((setSum, set) => setSum + set.weight * set.reps, 0),
    0
  );
  const cardioMinutes = completedSessions.reduce((sum, session) => {
    const hasCardio = session.setLogs.some((set) => {
      const exercise = exercises.find((item) => item.id === set.exerciseId);
      return exercise?.category === "conditioning";
    });
    return sum + (hasCardio ? session.durationMinutes ?? 0 : 0);
  }, 0);

  const prs = exercises
    .map((exercise) => {
      const best = completedSessions
        .flatMap((session) => session.setLogs.map((set) => ({ ...set, date: session.date })))
    ...
    .filter(Boolean)
    .slice(0, 5) as ProgressSummary["prs"];
...
export function getProgressionHint(exerciseId: string, sessions: WorkoutSession[], repMax: number) {
  ...
  if (recentSets.length < 2) return "Log a few sets to unlock a specific progression cue.";
